...
public class Effects {
	...
	public static int[][] flip(int[][] source) {
		...
		for (int x = 0 ; x < width ; x++) {
			for (int y = 0 ; y < height ; y++) {
				//output[x][y] = source[x * width  ,width][y * height / newHeight];
	...
	public static int[][] redeye(int[][] source, int[][] sourceB) {
		...
		for(int x =0 ; x<width;x++){
			for(int y = 0; y <height;y++){
				int red = redValue(source[x][y]);
				int green = greenValue(source[x][y]);
				int blue = blueValue(source[x][y]);
				int average = (red+blue+green)/3;
				output[x][y] = rgbValue(red+average,blue-average*5,green-average*5);
	...
	public static int[][] brighten(int[][] source) {
		...
		for(int x =0 ; x<width;x++){
			for(int y = 0; y <height;y++){
				...
				int blue = blueValue(source[x][y]);
				int average = (red+blue+green)/3;
				if(red+average/5 <= 255 & blue+average/5 <=255 & green+average/5 <= 255){
				output[x][y] = rgbValue(red+average/5,blue+average/5,green+average/5);
				//red+average/5 <= 255||blue+average/5 <=255 ||green+average/5 <= 255);
				}
			}
		}
		return output;
	...
	public static int[][] fade(int[][] source) {
		...
		for(int x =0 ; x<width;x++){
			for(int y = 0; y <height;y++){
				...
				int blue = blueValue(source[x][y]);
				int average = (red+blue+green)/3;
				output[x][y] = rgbValue(red-average,blue-average,green-average);
	...
	public static int[][] vignette2(int[][] source) {
		int width= source.length;
		int height  = source[0].length;
		int[][] output = new int[width][height];
		for(int x =0 ; x<width;x++){
			for(int y = 0; y <height;y++){
				...
				int blue = blueValue(source[x][y]);
				int average = (red+blue+green)/3;
				//output[x][y] = rgbValue(red-(x*y)/average,blue-(x*y)/average,green-(x*y)/average);
				//output[x][y] = rgbValue(red-(Math.abs(Math.sqrt(Math.))),blue-(Math.abs(x-width/2)),green-Math.abs(x-width/2));
				output[x][y] = rgbValue(red-getDistance(x-width/2,y-height/2),green-getDistance(x-width/2,y-height/2),blue-getDistance(x-width/2,y-height/2));
				//output[x][y] = rgbValue(red+getDistance(x-width,y-height),green+getDistance(x-width,y-height),blue+getDistance(x-width,y-height));
			}
		}
		return output;
	}
	...
	public static int[][] vignette(int[][] source) {
		...
		for(int x =0 ; x<width;x++){
			for(int y = 0; y <height;y++){
				int red = redValue(source[x][y]) - getDistance(x-width/2,y-height/2);
				int green = greenValue(source[x][y]) - getDistance(x-width/2,y-height/2);
				int blue = blueValue(source[x][y]) -getDistance(x-width/2,y-height/2);
				if (red < 0) {
					red = 0;
				}
				if (green < 0) {
				...
				//output[x][y] = rgbValue(red-(x*y)/average,blue-(x*y)/average,green-(x*y)/average);
				//output[x][y] = rgbValue(red-(Math.abs(Math.sqrt(Math.))),blue-(Math.abs(x-width/2)),green-Math.abs(x-width/2));
				//output[x][y] = rgbValue(red,green-getDistance(x-width/2,y-height/2),blue-getDistance(x-width/2,y-height/2));
				//output[x][y] = rgbValue(red+getDistance(x-width,y-height),green+getDistance(x-width,y-height),blue+getDistance(x-width,y-height));
